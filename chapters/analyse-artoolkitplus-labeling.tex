...
Das asymptotische Wachstum für $T(\mathit{lxsize}) = 6\mathit{lxsize} + 1$ ist, für die Werte $c_{1} = 7$,
 $c_{2} = 7$ und $n_{0} = 1$, $6\mathit{lxsize} + 1 = \Theta(lxsize)$. Für $T(\mathit{lysize}) = 8\mathit{lysize} + 9$,
 mit den Werten $c_{1} = 17$, $c_{2} = 17$ und $n_{0} = 1$, ist das Wachstum $8\mathit{lysize} + 9 = \Theta(lysize)$.
