...
Das asymptotische Wachstum für $T(\mathit{lxsize}) = 6\mathit{lxsize} + 1$ ist, für die Werte $c_{1} = 6$,
 $c_{2} = 7$ und $\mathit{lxsize}_{0} \geq 1$, $6\mathit{lxsize} + 1 = \Theta(lxsize)$. Für
 $T(\mathit{lysize}) = 8\mathit{lysize} + 9$, mit den Werten $c_{1} = 7$, $c_{2} = 9$ und $\mathit{lysize}_{0} \geq 9$,
...
In Zeile \ref{alg:arlabeling-neighbour-case2-1-n} wird der Wert $N_2$ in $n$ hinterlegt. In Zeile
 ...
 $\mathit{wk\_max}_{0} \geq 14$ ist $5 \mathit{wk\_max} + 14 = \Theta(n)$.
...
Dazu wird in Zeile
 ...
 gleichen Prinzip initialisiert. Die Laufzeitfunktion ist in \autoref{eq:analyse-arlabeling-sortlabels-initlabelmemory}
 angegeben. Für die Werte $c_{1} = 8$, $c_{2} = 9$ und $\mathit{size}_0 \geq 12$ ist
 $8\mathit{size} + 12 = \Theta(\mathit{size})$.
\input{eq/artplus/arlabeling-sortlabels-initlabelmemory}
...
Dazu wird in Zeile \ref{alg:arlabeling-calcregiondata-wclip-start}--\ref{alg:arlabeling-calcregiondata-wclip-end} der
 ...
 \textproc{arLabeling} (\autoref{alg:arlabeling-overview}) abgeschlossen. Die Laufzeitfunktion für Zeile
...
\input{eq/artplus/arlabeling-calcregiondata-1}
 ...
 \autoref{eq:analyse-arlabeling-calcregiondata-2} aufgeführt.
\input{eq/artplus/arlabeling-calcregiondata-2}
Für $c_{1} = 90$, $c_{2} = 91$ und $\mathit{wk\_max}_0 \geq 1$ ist die Wachstumsrate
 $90\mathit{wk\_max} + 1 = \Theta(wk\_max)$. Die letzte Laufzeitfunktion von \autoref{alg:arlabeling-calcregiondata} ist
 in \autoref{eq:analyse-arlabeling-calcregiondata-2} angegeben.
\input{eq/artplus/arlabeling-calcregiondata-3}
Die Wachstumsrate entspricht $20\mathit{labelnum} + 6 = \Theta(\mathit{labelnum})$, für $c_{1} = 20$, $c_{2} = 21$ und $\mathit{labelnum}_0 \geq 6$.
