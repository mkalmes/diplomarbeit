...
Um aus den gefundenen \gls{edgel} Liniensegmente zu erzeugen, wird \autoref{alg:findlinesegments1} verwendet. Das
...
% Durch Subsitution von $n = \tfrac{1}{5}(\mathit{regionSize}^2 -2\mathit{regionSize})$ in
%  \autoref{eq:findlinesegments-3-4} ist die Wachstumsrate des Verfahrens $\tfrac{21}{5}(r^2 - 2r) = \Theta(r^2)$, für
%  $c_{1} = 4$, $c_{2} = 5$ und $r_{0} = 42$.
...
% Wie bei \autoref{alg:findlinesegments5} auch, entspricht die Wachstumsrate nach der Subsitution
%  $\tfrac{21r^2}{5} - \frac{42r}{5} = \Theta(r^2)$, für $c_{1} = 4$, $c_{2} = 5$ und $r_{0} = 42$. Die Wachstumsrate von
%  \autoref{alg:findlinesegments5} und \autoref{alg:findlinesegments6} ist somit
%  $\Theta(r^2) + \Theta(r^2) = \Theta(r^2)$.
...
Die Kosten der  Algorithmen sind zur Analyse des schlechtesten Falls in \autoref{alg:findlinesegments9} aufgeführt. Die
 Laufzeitfuntion des RANSAC Verfahrens ist in \autoref{} angegeben.
