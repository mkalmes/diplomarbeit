...
\begin{algorithmic}[1]
	...
	\label{alg:detectmarker2-2-loop-start}
		...
		\Cost{$c_{18}$}{$10\mathit{label\_num}$}
		...
		\end{aligned}\right)$
		\Cost{$c_{21}$}{$t \cdot \mathit{label\_num}$}
		\label{alg:detectmarker2-2-callcontour}
		\State \ldots
		\State $i \gets i + 1$
		\Cost{$c_{22}$}{$\mathit{label\_num}$}
	\EndFor
