	...
	\begin{algorithmic}[1]
		...
		\For{$x \gets \mathit{left}$ \textbf{to} $x < \mathit{left} + \mathit{regionSize}$}
			...
			\For{$y \gets \mathit{top}$ \textbf{to} $y < \mathit{top} + \mathit{regionSize}$}
				\State $\mathit{currentEdgel} \gets$ \Call{convoluteKernelY}{$I,x,y,\mathit{blue},\mathit{imageWidth}, \mathit{imageHeight}$}
				\State $\mathit{red} \gets$ \Call{convoluteKernelY}{$I,x,y,\mathit{red},\mathit{imageWidth}, \mathit{imageHeight}$}
				\State $\mathit{green} \gets$ \Call{convoluteKernelY}{$I,x,y,\mathit{green},\mathit{imageWidth}, \mathit{imageHeight}$}
